...
function UserLaporanPage() {
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      setError(
        err.response?.data?.message || "Gagal memuat riwayat laporan Anda",
      );
    } finally {
      setLoading(false);
    }
    ...
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center text-center">
        <div className="mb-6 flex size-28 items-center justify-center rounded-full bg-linear-to-br from-indigo-50 to-blue-100">
          <svg
            className="size-14 text-indigo-600"
            fill="none"
            stroke="currentColor"
            strokeWidth="1.5"
            viewBox="0 0 24 24"
        ...
        <p className="mb-8 max-w-sm text-sm text-gray-500">
          Anda belum pernah melaporkan tumpukan sampah ilegal. Mari bantu
    ...
    <div className="relative space-y-6">
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-5">
          {items.map((item) => {
            const st = STATUS_LABELS[item.status_laporan?.toLowerCase()] || {
              text: item.status_laporan,
              bg: "bg-gray-50",
              color: "text-gray-700",
              ...
              >
                ...
                <div className="relative h-48 shrink-0 overflow-hidden bg-gray-100 sm:w-56">
                  ...
                  <div className="absolute top-3 left-3">
                    <span
                      className={`inline-flex items-center rounded-full px-2.5 py-1 text-[11px] font-bold ring-1 ring-inset ${st.bg} ${st.color} ${st.ring}`}
                    >
                      {st.text}
                    </span>
                  </div>
                ...
                <div className="flex flex-1 flex-col p-5 sm:p-6">
                  ...
                  <p className="mt-4 line-clamp-2 max-w-2xl text-[13px] leading-relaxed text-gray-600">
                    {item.deskripsi_laporan ||
                      "Tidak ada rincian deskripsi mengenai laporan ini."}
                  </p>
