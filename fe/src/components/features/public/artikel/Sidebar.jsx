...
export const Sidebar = ({
  ...
  return (
    <>
      ...
      <aside className="hidden h-fit border-l border-gray-100 pl-8 lg:block">
        <div className="w-full">
          ...
          {isLoggedIn ? (
            ...
            <div className="mb-7 rounded-xl border border-dashed border-gray-200 bg-gray-50 p-4 text-center">
            ...
            <div className="mb-10">
              ...
              <div className="flex flex-col gap-5">
                {popularArticles.map((item, idx) => (
                  <div key={item.id} className="group flex cursor-pointer items-start gap-3">
                    ...
                    <div>
                      <h4 className="mb-1.5 line-clamp-2 text-sm font-bold leading-snug text-gray-900 transition-colors group-hover:text-blue-600">
                        {item.title}
                      </h4>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[11px] font-medium text-gray-500">
                          {item.author} <span className="mx-0.5">•</span> {item.date}
                        </span>
                      </div>
                    </div>
                  </div>
          ...
          {topics && topics.length > 0 && (
            <div className="mb-10">
              <h3 className="mb-4 text-base font-bold text-gray-900">Jelajahi Topik</h3>
      ...
      {/* ══════════════════════════════════════════════════════════
          DRAWER KOMENTAR (hanya di halaman detail)
      ══════════════════════════════════════════════════════════ */}
      {isDetail && (
        <>
          {/* Overlay — z-[9998] */}
          ...
          {/* Panel — z-[9999] */}
          <div
            className={`fixed right-0 top-0 z-[9999] h-full w-full max-w-[400px] bg-white shadow-2xl transition-transform duration-300 ease-in-out ${
              ...
              <div className="flex-1 overflow-y-auto px-6 py-6">
                ...
                {isLoggedIn ? (
                  <div className="mb-8 rounded-xl border border-gray-200 bg-white p-4 shadow-sm transition-all focus-within:border-gray-400 focus-within:shadow-md">
                    <div className="mb-3 flex items-center gap-3">
                      ...
                      />
                      <span className="text-sm font-medium text-gray-800">Anda</span>
                    </div>
                    <textarea
                      value={komentarTeks}
                      onChange={onKomentarChange}
                      disabled={komentarLoading}
                      className="min-h-[80px] w-full resize-none border-none text-sm text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-0 disabled:opacity-60"
                      placeholder="What are your thoughts?"
                    />
                    {komentarError && (
                      <p className="mb-2 text-xs font-medium text-red-500">{komentarError}</p>
                    )}
                    <div className="mt-2 flex items-center justify-between pt-2">
                      <div className="flex gap-3 text-gray-400">
                        <button className="font-serif text-lg font-bold hover:text-gray-700">B</button>
                        <button className="font-serif text-lg italic hover:text-gray-700">i</button>
                      </div>
                      <div className="flex items-center gap-3">
                        <button onClick={onCloseComment} className="text-sm font-medium text-gray-500 hover:text-gray-800">
                          Cancel
                        </button>
                        <button
                ...
                {comments.length > 0 && (
                  <div className="mb-6 flex items-center border-b border-gray-100 pb-4">
                    <span className="cursor-pointer text-xs font-bold uppercase tracking-wide text-gray-900 transition-colors hover:text-gray-600">
                ...
                <div className="space-y-8">
                  {comments.map((c, i) => {
                    const nama = c.user?.full_name ?? c.user?.username ?? (typeof c.user === "string" ? c.user : "Anonim");
                    const avatarUrl = c.user?.avatar_url ?? c.userImage ?? `https://ui-avatars.com/api/?name=${encodeURIComponent(nama)}&background=random&size=32`;
                    const teks = c.teks ?? c.text ?? "";
                    return (
                      <div key={c.id ?? i} className="group">
                        <div className="mb-3 flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            <img src={avatarUrl} alt={nama} className="h-8 w-8 rounded-full object-cover shadow-sm" />
                            <div>
                              <p className="text-[13px] font-bold text-gray-900">{nama}</p>
                              <p className="text-[11px] font-medium text-gray-500">Baru saja</p>
                            </div>
                          </div>
                        ...
                        <p className="text-[14px] leading-relaxed text-gray-800">{teks}</p>
                        <div className="mt-4 flex items-center justify-between border-t border-gray-50 pt-3 text-gray-400">
                          ...
                          <span className="cursor-pointer text-[12px] font-medium transition-colors hover:text-gray-700">Reply</span>
                        </div>
                      </div>
                    );
                  })}
